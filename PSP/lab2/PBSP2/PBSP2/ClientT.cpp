...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	try {
		...
		if ((connect(SocketTCP, (sockaddr*)&Server_IN, sizeof(Server_IN))) == SOCKET_ERROR) { //сокет для соединения
		...
		//	all.sin_addr.s_addr = inet_addr("127.0.0.1"); //inet_addr("127.0.43.255")
		...
		default:
			if (closesocket(SocketTCP) == SOCKET_ERROR)
				throw SetErrorMsgText("Closesocket:", WSAGetLastError()); throw "Введен неверный код;";
		}
		printf_s("\n ---------- ---------- ---------- ---------- \n");
...
//   // serv.sin_addr.s_addr = inet_addr("192.168.156.179");
//
//    try {
//        cout << "ClientT\n\n";
//
