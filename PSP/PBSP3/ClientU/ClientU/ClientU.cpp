...
int main()
{
    ...
    try {
        ...
        serv.sin_addr.s_addr = inet_addr("192.168.105.235");                
        ...
        for (int i = 1; i <= countMessage; i++) {
            string obuf = "Hello from Client1 " + to_string(i);
  
          
            if ((libuf = sendto(cC, obuf.c_str(), obuf.length() + 1, NULL, (SOCKADDR*)&serv, sizeof(serv))) == SOCKET_ERROR) //возвращает  количество пересланных байт данных 
            {
