//задание 1
var numb:number[] = new Array(3, 9, 3, 9, 9, 9, 1,2,3, 6);
function disp(arr_numbers: number[]) : string 
 {
    let str="(";
    for (var i:number = 0; i < arr_numbers.length; i++) {
        str+=arr_numbers[i];
...
function find_multiples(value: number) : number | null
{ 
    var sum:number=0;
...
for(var j in nums) { 
 ...
 var nums1:number[] = [1,3];
 var nums2:number[] = [2,4];
...
var sudoku:number[][] =[[6,3,8,4,2,7,9,1,5],
                        [5,9,1,8,6,3,4,2,7],
                        [4,2,7,9,1,5,8,6,3],
...
let check :Set<number>= new Set();
let check_kvadr1:Set<number> = new Set();
let check_kvadr2:Set<number> = new Set();
let check_kvadr3:Set<number>= new Set();

for (var i:number = 0; i < 9 && !fail; ++i) {
